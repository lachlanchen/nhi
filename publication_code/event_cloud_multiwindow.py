...
Only a sampled subset of events is plotted to keep the scatter legible.
"""
...
def sample_events(x, y, t, p, fraction: float, max_points: int = 500_000):
    n = len(x)
...
def plot_cloud(ax, x, y, t_ms, p, title: str, time_scale: float):
    ...
    ax.scatter(x[pos], y[pos], time_scale * t_ms[pos], c=colors["pos"], s=0.1, alpha=0.6, marker=".")
    ax.scatter(x[neg], y[neg], time_scale * t_ms[neg], c=colors["neg"], s=0.1, alpha=0.6, marker=".")
    ax.set_xlabel("X (px)")
    ax.set_ylabel("Y (px)")
...
def main():
    ...
    ap.add_argument("--sample", type=float, default=0.05, help="Fraction of events to plot (default: 0.05)")
    ap.add_argument("--time-scale", type=float, default=1.5, help="Stretch factor for time axis (default: 1.5)")
    ap.add_argument("--chunk-size", type=int, default=500000, help="Chunk size for compensation (default: 500000)")
    ...
    out_dir = args.output_dir / f"event_cloud_multiwindow_{timestamp}"
