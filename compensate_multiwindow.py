...
class Compensate(nn.Module):
    ...
    def compute_event_compensation(self, x, y, t_shifted):
        ...
        a_expanded = self.a_params[:, None].expand(-1, num_events)  # [13, num_events]
        b_expanded = self.b_params[:, None].expand(-1, num_events)  # [13, num_events]
        ...
        a_lower = self.a_params[:-1, None].expand(-1, num_events)  # [12, num_events]
        a_upper = self.a_params[1:, None].expand(-1, num_events)   # [12, num_events]
        b_lower = self.b_params[:-1, None].expand(-1, num_events)  # [12, num_events]
        b_upper = self.b_params[1:, None].expand(-1, num_events)   # [12, num_events]
...
class ScanCompensation(nn.Module):
    def __init__(self, duration, device='cuda'):
        super().__init__()
        
        # Initialize 13 parameters each for a_x and a_y (for 10 main windows)
        ...
        a_params = [0.0] * 13  # Initialize to zero
        b_params = [0.0] * 13  # Initialize to zero
        
        # Create the compensate object - it will create its own Parameters
        self.compensate = Compensate(a_params, b_params, duration, device=device)
...
def train_scan_compensation(x, y, t, p, sensor_width=1280, sensor_height=720, 
    ...
    """
    Train the multi-window scan compensation model with intelligent subsampling
    """
    ...
    # Initialize multi-window compensation model
    model = ScanCompensation(duration.item(), device=device)

    # Define the optimizer - now optimizes 26 parameters (13 a + 13 b)
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)

    print(f"\nTraining Progress:")
    print(f"{'Iter':<6} {'Loss':<12} {'a_range':<18} {'b_range':<18}")
    print("-" * 60)
    ...
    for i in range(num_iterations):
        ...
        current_a_params = model.compensate.a_params.detach().cpu().numpy().copy()
        current_b_params = model.compensate.b_params.detach().cpu().numpy().copy()
        ...
        if i % 100 == 0 or i == num_iterations - 1:  # Every 100 iterations + final
            a_range = f"[{current_a_params.min():.4f}, {current_a_params.max():.4f}]"
            b_range = f"[{current_b_params.min():.4f}, {current_b_params.max():.4f}]"
            print(f"{i:<6} {current_loss:<12.6f} {a_range:<18} {b_range:<18}")
    ...
    return model, losses, a_params_history, b_params_history, original_t_start, original_t_end

def plot_learned_parameters(model, sensor_width, sensor_height, output_dir=None, filename_prefix=""):
    """
    Plot initial (all zeros) and final learned parameters in X-T and Y-T planes with dotted boundary lines
    """
    print("Creating learned parameters visualization...")
    
    # Get final parameters
    final_a_params = model.compensate.a_params.detach().cpu().numpy()
    final_b_params = model.compensate.b_params.detach().cpu().numpy()
    
    # Create initial parameters (all zeros)
    initial_a_params = np.zeros_like(final_a_params)
    initial_b_params = np.zeros_like(final_b_params)
    ...
    # Helper function to compute boundary lines using the event-wise method
    ...
    # Plot 1: Initial parameters X-T view
    ax = axes[0, 0]
    initial_x_lines = compute_boundary_lines_simple(initial_a_params, initial_b_params, x_range, 'x')
    
    for i, line_values in enumerate(initial_x_lines):
        color = colors[i % len(colors)]
        ax.plot(x_range, line_values/1000, '--', alpha=0.7, linewidth=2, color=color, label=f'Boundary {i}')
    
    ax.set_xlabel('X (pixels)')
    ax.set_ylabel('Time (ms)')
    ax.set_title('Initial Parameters - X-T View\n(All zeros - no compensation)')
    ax.grid(True, alpha=0.3)
    ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    ax.set_ylim(0, duration/1000)
    
    # Plot 2: Final parameters X-T view
    ax = axes[0, 1] 
    ...
    ax.set_xlabel('X (pixels)')
    ax.set_ylabel('Time (ms)')
    ax.set_title('Final Learned Parameters - X-T View')
    ax.grid(True, alpha=0.3)
    ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    ax.set_ylim(0, duration/1000)
    
    # Plot 3: Initial parameters Y-T view
    ax = axes[1, 0]
    initial_y_lines = compute_boundary_lines_simple(initial_a_params, initial_b_params, y_range, 'y')
    
    for i, line_values in enumerate(initial_y_lines):
        color = colors[i % len(colors)]
        ax.plot(y_range, line_values/1000, '--', alpha=0.7, linewidth=2, color=color, label=f'Boundary {i}')
    
    ax.set_xlabel('Y (pixels)')
    ax.set_ylabel('Time (ms)')
    ax.set_title('Initial Parameters - Y-T View\n(All zeros - no compensation)')
    ax.grid(True, alpha=0.3)
    ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    ax.set_ylim(0, duration/1000)
    
    # Plot 4: Final parameters Y-T view
    ax = axes[1, 1]
    ...
    ax.set_xlabel('Y (pixels)')
    ax.set_ylabel('Time (ms)')
    ax.set_title('Final Learned Parameters - Y-T View')
    ax.grid(True, alpha=0.3)
    ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    ax.set_ylim(0, duration/1000)
    
    # Add parameter statistics as text
    fig.text(0.02, 0.02, 
             f'Final parameters summary:\n'
             f'a_params: [{final_a_params.min():.4f}, {final_a_params.max():.4f}] (range: {final_a_params.max()-final_a_params.min():.4f})\n'
             f'b_params: [{final_b_params.min():.4f}, {final_b_params.max():.4f}] (range: {final_b_params.max()-final_b_params.min():.4f})\n'
             f'Main windows: {model.compensate.num_main_windows}, Total windows: {model.compensate.num_total_windows}',
             fontsize=10, fontfamily='monospace',
             bbox=dict(boxstyle='round', facecolor='lightgray', alpha=0.8))
    
    plt.suptitle('Multi-Window Compensation: Learned Boundary Lines', fontsize=16, y=0.98)
    plt.tight_layout()
    
    if output_dir:
        plot_path = os.path.join(output_dir, f"{filename_prefix}_learned_parameters.png")
        plt.savefig(plot_path, dpi=150, bbox_inches='tight')
        print(f"Learned parameters plot saved to: {plot_path}")
    
    plt.show()
    
    # Print parameter summary
    print(f"\nLearned Parameters Summary:")
    print(f"  a_params: {final_a_params}")
    print(f"  b_params: {final_b_params}")
...
def create_event_frames(model, x, y, t, p, H, W, bin_width, original_t_start, original_t_end, compensated=True):
    ...
    with torch.no_grad():
        if compensated:
            ...
            # Temporarily set all parameters to zero
...
def get_param_string(model):
    ...
    """
    a_params = model.compensate.a_params.detach().cpu().numpy()
    b_params = model.compensate.b_params.detach().cpu().numpy()
    return f"a_range=[{a_params.min():.4f},{a_params.max():.4f}], b_range=[{b_params.min():.4f},{b_params.max():.4f}]"
...
def get_param_suffix(model):
    ...
    a_params = model.compensate.a_params.detach().cpu().numpy()
    b_params = model.compensate.b_params.detach().cpu().numpy()
    return f"_multiwindow_a{a_params.min():.4f}_{a_params.max():.4f}_b{b_params.min():.4f}_{b_params.max():.4f}"

def visualize_results(model, x, y, t, p, losses, a_params_history, b_params_history, bin_width, 
    ...
    axes[1, 1].text(0.1, 0.75, f'a_params range: [{final_a_params.min():.4f}, {final_a_params.max():.4f}]', transform=axes[1, 1].transAxes, fontsize=9)
    axes[1, 1].text(0.1, 0.70, f'b_params range: [{final_b_params.min():.4f}, {final_b_params.max():.4f}]', transform=axes[1, 1].transAxes, fontsize=9)
    axes[1, 1].text(0.1, 0.60, f'Final loss: {losses[-1]:.6f}', transform=axes[1, 1].transAxes, fontsize=10)
    axes[1, 1].text(0.1, 0.55, f'Total events: {len(x):,}', transform=axes[1, 1].transAxes, fontsize=10)
    axes[1, 1].text(0.1, 0.50, f'Time bins: {actual_num_bins}', transform=axes[1, 1].transAxes, fontsize=10)
    axes[1, 1].text(0.1, 0.45, f'Main windows: {model.compensate.num_main_windows}', transform=axes[1, 1].transAxes, fontsize=10)
    axes[1, 1].text(0.1, 0.40, f'Total parameters: {len(final_a_params) + len(final_b_params)}', transform=axes[1, 1].transAxes, fontsize=10)
    axes[1, 1].set_title('Multi-Window Summary')
    axes[1, 1].set_xticks([])
    axes[1, 1].set_yticks([])
    
    # Add overall title with parameters
    fig.suptitle(f'Multi-Window Scan Compensation Results\n{param_str}', fontsize=16, y=0.98)
...
def save_results(model, losses, a_params_history, b_params_history, output_dir, filename_prefix):
    ...
    if output_dir:
        ...
        with open(results_path, 'w') as f:
            ...
            f.write(f"Total parameters: {len(final_a_params) + len(final_b_params)}\n")
...
def main():
    ...
    # Train multi-window model
    model, losses, a_params_history, b_params_history, original_t_start, original_t_end = train_scan_compensation(
        x, y, t, p,
    ...
    final_b_params = model.compensate.b_params.detach().cpu().numpy()
    print(f"\nFinal Results:")
    print(f"  a_params range: [{final_a_params.min():.6f}, {final_a_params.max():.6f}]")
    print(f"  b_params range: [{final_b_params.min():.6f}, {final_b_params.max():.6f}]")
    print(f"  Final loss: {losses[-1]:.6f}")
    print(f"  Parameters optimized: {len(final_a_params) + len(final_b_params)}")
    ...
    if args.plot_params:
        plot_learned_parameters(model, args.sensor_width, args.sensor_height, args.output_dir, base_name)
    
    # Visualize if requested
    if args.visualize:
        visualize_results(model, x, y, t, p, losses, a_params_history, b_params_history, 
