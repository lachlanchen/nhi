...
// Acceleration parameters - reduced for immediate acceleration
#define ACCELERATION_STEPS 0   // Minimal steps to start smoothly
#define DECELERATION_STEPS 0   // Minimal steps to stop smoothly
#define MIN_STEP_DELAY 500     // Minimum delay between steps in microseconds (maximum speed)
#define START_STEP_DELAY 2000  // Brief startup delay in microseconds
...
// Modified move_motor_degrees function with immediate acceleration to constant speed
void move_motor_degrees(float degrees, float rpm) {
    ...
    // Calculate target step delay based on RPM (constant speed)
    ...
    // Minimal acceleration/deceleration steps for smooth start/stop
    ...
    // Movement loop with immediate acceleration to constant speed
    for (long i = 0; i < steps_to_move; i++) {
        ...
        // Immediate acceleration: use brief startup delay for first few steps, then constant speed
        if (i < accel_steps) {
            // Very brief acceleration - quickly reach target speed
            step_delay = START_STEP_DELAY - ((START_STEP_DELAY - target_step_delay) * i / accel_steps);
        } else if (i >= (steps_to_move - decel_steps)) {
            // Very brief deceleration - quickly slow down
            long decel_step = steps_to_move - i;
            step_delay = target_step_delay + ((START_STEP_DELAY - target_step_delay) * (decel_steps - decel_step) / decel_steps);
        } else {
            // Constant speed for the majority of the movement
            step_delay = target_step_delay;
        }
