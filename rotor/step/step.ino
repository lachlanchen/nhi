...
void x_limit_reached() {
    digitalWrite(EN_PIN, LOW);  // Disable the motor
    Serial.println("X Limit Reached - Motor Stopped");
}

void y_limit_reached() {
    digitalWrite(EN_PIN, LOW);  // Disable the motor
    Serial.println("Y Limit Reached - Motor Stopped");
}
...
void move_motor_sequence() {
  for (int i = 0; i < 5; i++) {
    // Move 30 degrees positively
    digitalWrite(DIR_PIN, LOW);
    move_motor_degrees(90);
    delay(1000); // Wait for 1 second

    // Move 30 degrees negatively
    digitalWrite(DIR_PIN, HIGH);
    move_motor_degrees(90);
    delay(1000); // Wait for 1 second
  }
