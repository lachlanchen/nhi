...
def visualize_3d_membership_bands(comp, x_range, y_range, t_range):
    """
    Visualize 3D membership as bands/boxes in 3D space
    Shows windows 1, 3, 5, 7, 9 (main windows) as highlighted 3D regions
    """
    print("Visualizing 3D membership bands...")
    ...
    for i, window_idx in enumerate(windows_to_show):
        ...
        membership = memberships[window_idx]  # Shape: [x, y, t]
        
        # Find regions where membership is high (> 0.5)
        high_membership = membership > 0.5
        
        # Get coordinates where membership is high
        x_coords, y_coords, t_coords = np.where(high_membership)
        
        if len(x_coords) > 0:
            # Map indices back to actual coordinates
            x_vals = x_range[x_coords]
            y_vals = y_range[y_coords]  
            t_vals = t_range[t_coords]
            
            # Color by membership strength
            membership_vals = membership[high_membership]
        ...
        ax.set_zlabel('T (ms)')
        ax.set_title(f'Window {window_idx} Membership > 0.5\n(Main Window {window_idx-1})')
        
        # Set consistent view
        ax.set_xlim(x_range.min(), x_range.max())
        ax.set_ylim(y_range.min(), y_range.max())
        ax.set_zlim(t_range.min()/1000, t_range.max()/1000)
        
        # Print window statistics
        total_points = membership.size
        high_points = np.sum(high_membership)
        avg_membership = membership.mean()
        max_membership = membership.max()
        
        print(f"Window {window_idx}: {high_points}/{total_points} points > 0.5, "
              f"avg={avg_membership:.3f}, max={max_membership:.3f}")
    ...
    for i, window_idx in enumerate(windows_to_show):
        membership = memberships[window_idx]
        high_membership = membership > 0.3  # Lower threshold for overview
        
        x_coords, y_coords, t_coords = np.where(high_membership)
        
        if len(x_coords) > 0:
            x_vals = x_range[x_coords[::5]]  # Subsample for performance
            y_vals = y_range[y_coords[::5]]
            t_vals = t_range[t_coords[::5]]
    ...
    # Set consistent view
    ax6.set_xlim(x_range.min(), x_range.max())
    ax6.set_ylim(y_range.min(), y_range.max())
    ax6.set_zlim(t_range.min()/1000, t_range.max()/1000)
    
    plt.tight_layout()
    plt.show()
    
    print(f"\n3D Membership Bands Visualization:")
    print(f"✓ Each plot shows 3D regions where that window has membership > 0.5")
    print(f"✓ Points are colored by membership strength (darker = higher)")
    print(f"✓ You should see distinct 3D 'bands' or 'boxes' for each window")
