...
Enhanced with file caching, auto-loading, projection views, data preprocessing with presets, and improved defaults
"""
...
def apply_preset_compensation(x, y, t, p, preset_ax, preset_ay):
    """Apply preset compensation to the timestamp data"""
    if preset_ax == 0.0 and preset_ay == 0.0:
        return x, y, t, p  # No preprocessing needed
    
    # Apply preset compensation: t_new = t - preset_ax * x - preset_ay * y
    t_compensated = t - preset_ax * x - preset_ay * y
    
    return x, y, t_compensated, p
...
class ScanCompensation(nn.Module):
    ...
    def warp(self, x_coords, y_coords, timestamps):
        """Adjust timestamps based on x and y positions (normal optimization)"""
        a_x = self.params[0]
        a_y = self.params[1]
...
class ScanCompensationGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Scan Compensation with 3D Spectral Visualization & Data Presets")
        ...
        # Data storage
        self.original_events_data = None  # Original loaded data
        self.events_data = None           # Preprocessed data (after preset compensation)
        self.model = None
        self.compensation_params = None
        self.compensated_events = None
        self.preset_ax = 0.0              # Store current preset values
        self.preset_ay = 0.0
    ...
    def save_config(self):
        ...
        try:
            self.config.update({
                ...
                "preset_ax": self.preset_ax_var.get(),
                "preset_ay": self.preset_ay_var.get(),
    ...
    def create_widgets(self):
        ...
        file_frame.grid(row=0, column=0, columnspan=6, sticky="ew", padx=2, pady=2)
        ...
        # Data Preprocessing Presets - Row 2, Column 1 (NEW)
        preset_frame = ttk.LabelFrame(control_frame, text="Data Preprocessing", padding=5)
        preset_frame.grid(row=1, column=0, sticky="ew", padx=2, pady=2)
        
        ttk.Label(preset_frame, text="Preset a_x:").grid(row=0, column=0, sticky="w")
        self.preset_ax_var = tk.DoubleVar(value=self.config["preset_ax"])
        ttk.Entry(preset_frame, textvariable=self.preset_ax_var, width=12).grid(row=0, column=1, padx=2)
        
        ttk.Label(preset_frame, text="Preset a_y:").grid(row=1, column=0, sticky="w")
        self.preset_ay_var = tk.DoubleVar(value=self.config["preset_ay"])
        ttk.Entry(preset_frame, textvariable=self.preset_ay_var, width=12).grid(row=1, column=1, padx=2)
        
        # Apply Presets Button
        ttk.Button(preset_frame, text="Apply Presets", command=self.apply_presets).grid(row=0, column=2, rowspan=2, padx=5, sticky="ns")
        
        # Manual Parameters - Row 2, Column 2
        manual_frame = ttk.LabelFrame(control_frame, text="Manual Parameters", padding=5)
        manual_frame.grid(row=1, column=1, sticky="ew", padx=2, pady=2)
        ...
        # Training Control - Row 2, Column 3
        training_frame = ttk.LabelFrame(control_frame, text="Training Control", padding=5)
        training_frame.grid(row=1, column=2, sticky="ew", padx=2, pady=2)
        ...
        # Optimization Parameters - Row 2, Column 4
        param_frame1 = ttk.LabelFrame(control_frame, text="Optimization", padding=5)
        param_frame1.grid(row=1, column=3, sticky="ew", padx=2, pady=2)
        ...
        # Visualization Parameters - Row 2, Column 5
        param_frame2 = ttk.LabelFrame(control_frame, text="Visualization", padding=5)
        param_frame2.grid(row=1, column=4, sticky="ew", padx=2, pady=2)
        ...
        # Actions - Row 2, Column 6
        action_frame = ttk.LabelFrame(control_frame, text="Actions", padding=5)
        action_frame.grid(row=1, column=5, sticky="ew", padx=2, pady=2)
        ...
        save_frame.grid(row=2, column=3, columnspan=3, sticky="ew", padx=2, pady=2)
        ...
        ttk.Button(save_frame, text="Auto Set Params", command=self.auto_set_params).grid(row=0, column=2, sticky="ew", padx=2)
        ttk.Button(save_frame, text="Clear Presets", command=self.clear_presets).grid(row=1, column=0, columnspan=3, sticky="ew", padx=2)
        ...
        # Log initialization
        self.log_result("Enhanced Scan Compensation GUI with Data Preprocessing Initialized")
        ...
        self.log_result("Data preprocessing: Apply presets to modify timestamps before optimization")
        
    def apply_presets(self):
        """Apply preset compensation to the loaded data"""
        if self.original_events_data is None:
            messagebox.showwarning("Warning", "Please load a file first")
            return
            
        try:
            x, y, t, p = self.original_events_data
            preset_ax = self.preset_ax_var.get()
            preset_ay = self.preset_ay_var.get()
            
            # Apply preset compensation to timestamps
            x_prep, y_prep, t_prep, p_prep = apply_preset_compensation(x, y, t, p, preset_ax, preset_ay)
            self.events_data = (x_prep, y_prep, t_prep, p_prep)
            
            # Store current preset values
            self.preset_ax = preset_ax
            self.preset_ay = preset_ay
            
            self.log_result(f"\nDATA PREPROCESSING APPLIED:")
            self.log_result(f"  Preset a_x = {preset_ax:.6f} μs/pixel")
            self.log_result(f"  Preset a_y = {preset_ay:.6f} μs/pixel")
            if preset_ax != 0.0 or preset_ay != 0.0:
                original_t_range = f"{t.min():.0f} - {t.max():.0f}"
                processed_t_range = f"{t_prep.min():.0f} - {t_prep.max():.0f}"
                self.log_result(f"  Original time range: {original_t_range} μs")
                self.log_result(f"  Processed time range: {processed_t_range} μs")
                self.log_result(f"  Data preprocessing complete - ready for optimization")
            else:
                self.log_result(f"  No preprocessing applied (zero presets)")
                
            self.update_status("Data preprocessing complete")
            
        except Exception as e:
            self.log_result(f"ERROR in data preprocessing: {str(e)}")
            messagebox.showerror("Error", f"Data preprocessing failed: {str(e)}")
    
    def clear_presets(self):
        """Clear preset parameters and revert to original data"""
        self.preset_ax_var.set(0.0)
        self.preset_ay_var.set(0.0)
        if self.original_events_data is not None:
            self.events_data = self.original_events_data  # Revert to original
            self.preset_ax = 0.0
            self.preset_ay = 0.0
            self.log_result("Preset parameters cleared - reverted to original data")
    ...
    def auto_load_worker(self):
        ...
        try:
            ...
            self.original_events_data = (x, y, t, p)
            
            # Apply any existing presets
            preset_ax = self.preset_ax_var.get()
            preset_ay = self.preset_ay_var.get()
            x_prep, y_prep, t_prep, p_prep = apply_preset_compensation(x, y, t, p, preset_ax, preset_ay)
            self.events_data = (x_prep, y_prep, t_prep, p_prep)
            ...
            # Auto-set parameters (normal optimization)
            ...
            if preset_ax != 0.0 or preset_ay != 0.0:
                self.log_result(f"Applied presets: ax={preset_ax:.6f}, ay={preset_ay:.6f}")
        ...
        def optimization_worker():
            try:
                ...
                if self.enable_training_var.get():
                    self.log_result("SCAN COMPENSATION OPTIMIZATION (ON PREPROCESSED DATA)")
                else:
                    self.log_result("MANUAL PARAMETER SETTING (ON PREPROCESSED DATA)")
                self.log_result("="*60)
                ...
                # Apply current preset compensation
                preset_ax = self.preset_ax_var.get()
                preset_ay = self.preset_ay_var.get()
                x_prep, y_prep, t_prep, p_prep = apply_preset_compensation(x, y, t, p, preset_ax, preset_ay)
                self.events_data = (x_prep, y_prep, t_prep, p_prep)
                ...
                self.log_result(f"Original time range: {t.min():.0f} - {t.max():.0f} μs")
                if preset_ax != 0.0 or preset_ay != 0.0:
                    self.log_result(f"Preset compensation applied: ax={preset_ax:.6f}, ay={preset_ay:.6f}")
                    self.log_result(f"Preprocessed time range: {t_prep.min():.0f} - {t_prep.max():.0f} μs")
                else:
                    self.log_result(f"No preset compensation applied")
                self.log_result(f"Duration: {(t_prep.max()-t_prep.min())/1e6:.3f} seconds")
                ...
                xs = torch.tensor(x_prep, device=device)
                ys = torch.tensor(y_prep, device=device)
                ts = torch.tensor(t_prep, device=device)
                ps = torch.tensor(p_prep, device=device)
                ...
                if self.enable_training_var.get():
                    ...
                    self.log_result(f"  Device: {device}")
                    self.log_result(f"  Operating on preprocessed data (preset-compensated)")
                    self.log_result(f"\nTraining progress:")
                    ...
                    total_ax = preset_ax + self.compensation_params[0]
                    total_ay = preset_ay + self.compensation_params[1]
                    
                    self.log_result(f"\nOPTIMIZATION COMPLETE!")
                    self.log_result(f"Learned parameters (on preprocessed data):")
                    self.log_result(f"  a_x = {self.compensation_params[0]:.6f} μs/pixel")
                    self.log_result(f"  a_y = {self.compensation_params[1]:.6f} μs/pixel")
                    self.log_result(f"Total effective compensation:")
                    self.log_result(f"  Total a_x = {preset_ax:.6f} + {self.compensation_params[0]:.6f} = {total_ax:.6f} μs/pixel")
                    self.log_result(f"  Total a_y = {preset_ay:.6f} + {self.compensation_params[1]:.6f} = {total_ay:.6f} μs/pixel")
                    ...
                    # Use manual parameters directly on preprocessed data
                    ...
                    self.log_result(f"Manual parameters (on preprocessed data):")
                    self.log_result(f"  a_x = {self.compensation_params[0]:.6f} μs/pixel")
                    self.log_result(f"  a_y = {self.compensation_params[1]:.6f} μs/pixel")
                    self.log_result(f"Total effective compensation:")
                    self.log_result(f"  Total a_x = {preset_ax:.6f} + {self.compensation_params[0]:.6f} = {total_ax:.6f} μs/pixel")
    ...
    def generate_3d_vis(self):
        ...
        try:
            ...
            self.log_result("\n" + "="*60)
            self.log_result("3D SPECTRAL VISUALIZATION (PREPROCESSED + OPTIMIZED)")
            self.log_result("="*60)
            
            x, y, t, p = self.events_data  # Use preprocessed data
            ...
            # Calculate total effective parameters
            total_ax = self.preset_ax + self.compensation_params[0]
            total_ay = self.preset_ay + self.compensation_params[1]
            
            self.log_result(f"Using preprocessed + optimized parameters:")
            self.log_result(f"  Data preprocessing: ax={self.preset_ax:.6f}, ay={self.preset_ay:.6f}")
            self.log_result(f"  Learned correction: ax={self.compensation_params[0]:.6f}, ay={self.compensation_params[1]:.6f}")
            self.log_result(f"  Total effective: ax={total_ax:.6f}, ay={total_ay:.6f}")
            ...
            if len(ts_sampled) > 0:
                ...
                if is_projection:
                    ...
                    if "XY" in view_name:
                        ...
                        # Add grid for XY projection
                        ax.grid(True, alpha=0.3)
                        ax.set_xticks(range(0, 1280, 200))
                        ax.set_yticks(range(0, 720, 100))
                        ...
                        # Add grid for YT projection
                        ax.grid(True, alpha=0.3)
                        ax.set_xticks(range(0, 720, 100))
                    ...
                    # Title with parameter info including preprocessing
                    mode = "Optimized" if self.enable_training_var.get() else "Manual"
                    total_ax = self.preset_ax + self.compensation_params[0]
                    total_ay = self.preset_ay + self.compensation_params[1]
                    preset_info = f"preset+learned" if (self.preset_ax != 0 or self.preset_ax != 0) else "learned only"
                    ax.set_title(f'3D Compensated Events - {mode} ({view_name})\n'
                               f'total_ax={total_ax:.3f}, total_ay={total_ay:.3f} ({preset_info}), '
                ...
                if is_projection:
                    self.log_result(f"Grid added to {view_name} projection")
