...
# Simplified script to process all recordings in subfolders
# Usage: ./process_all_recordings_simple.sh [OPTIONS]
...
show_usage() {
    ...
    echo ""
    echo "Simplified script to process all recordings in subfolders"
    echo ""
...
# Find all subfolders that contain .raw files
total_folders=0
successful=0
failed=0
failed_folders=()
...
for folder in */; do
    ...
    # Look for .raw files in this folder
    raw_files=()
    ...
    # Skip if no .raw files found
    if [[ ${#raw_files[@]} -eq 0 ]]; then
        echo "No .raw files found in $folder_name, skipping"
        continue
    fi
    
    # Process the first .raw file found (assuming one per folder)
    raw_file="${raw_files[0]}"
    base_name=$(basename "$raw_file" .raw)
    
    echo "Found: $raw_file in folder: $folder_name"
    
    ((total_folders++))
    
    # Process this recording
    if process_recording "$folder_name" "$raw_file" "$base_name"; then
        ((successful++))
    else
        ((failed++))
