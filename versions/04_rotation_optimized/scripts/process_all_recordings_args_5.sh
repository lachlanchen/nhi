...
#   -c, --skip-compensation         Skip the scan compensation step
#   -d, --base-dir DIR              Set base directory (default: ./plastics)
#   --skip FOLDER                   Skip processing specific folder(s) (can be used multiple times)
#   --only FOLDER                   Only process specific folder(s) (can be used multiple times)
#
...
#   ./process_all_recordings.sh --skip-compensation
#   ./process_all_recordings.sh --only pvc --only pet
#   ./process_all_recordings.sh --skip glassleft --skip glassright
...
show_usage() {
    ...
    echo "  -c, --skip-compensation         Skip the scan compensation step"
    echo "  -d, --base-dir DIR              Set base directory (default: $BASE_DIR_RELATIVE)"
    echo "  --skip FOLDER                   Skip processing specific folder(s) (can be used multiple times)"
    echo "  --only FOLDER                   Only process specific folder(s) (can be used multiple times)"
    echo ""
    ...
    echo "  $0 --skip-compensation"
    echo "  $0 --only pvc --only pet"
    echo "  $0 --skip glassleft --skip glassright"
    ...
    case $1 in
        ...
        --skip)
            if [[ -n "$2" && "$2" != -* ]]; then
                ...
                echo "Error: --skip requires a folder name"
                exit 1
            fi
            ;;
        --only)
...
if [[ "$SKIP_SEGMENTATION" == true && "$SKIP_COMPENSATION" == true ]]; then
    echo "Error: Cannot use both --skip-segmentation and --skip-compensation at the same time"
    echo "This would result in no processing steps being performed."
    exit 1
fi

if [[ ${#SKIP_FOLDERS[@]} -gt 0 && ${#ONLY_FOLDERS[@]} -gt 0 ]]; then
    echo "Error: Cannot use both --skip and --only options at the same time"
    echo "Use either --skip to exclude specific folders or --only to include specific folders"
    exit 1
fi
...
echo "  Skip compensation: $SKIP_COMPENSATION"
if [[ ${#SKIP_FOLDERS[@]} -gt 0 ]]; then
    echo "  Skip folders: ${SKIP_FOLDERS[*]}"
fi
if [[ ${#ONLY_FOLDERS[@]} -gt 0 ]]; then
    echo "  Only folders: ${ONLY_FOLDERS[*]}"
fi
echo ""
...
if [[ "$SKIP_COMPENSATION" == false && ! -f "$ALIGNMENT_SCRIPT" ]]; then
    echo "Error: Alignment script not found at $ALIGNMENT_SCRIPT"
    echo "Please adjust ALIGNMENT_SCRIPT path in this script or use --skip-compensation"
    exit 1
fi
...
process_recording() {
    ...
    # Step 2: Check if segments folder exists (only if we need it for compensation)
    if [[ "$SKIP_COMPENSATION" == false ]]; then
    ...
    # Step 3: Run scan compensation with merge (if not skipped)
    if [[ "$SKIP_COMPENSATION" == false ]]; then
        echo ""
        if [[ "$SKIP_SEGMENTATION" == false ]]; then
        ...
        echo ""
        echo "Step 2: Skipping scan compensation (--skip-compensation flag used)"
    fi
...
echo "Scanning for recording files..."

# Function to extract material name from filename
extract_material_name() {
    local filename="$1"
    # Extract material name from patterns like:
    # sync_recording_acc_fast_speed_3_pvc_event_20250726_073140.raw
    # or similar patterns
    if [[ "$filename" =~ _([^_]+)_event_ ]]; then
        echo "${BASH_REMATCH[1]}"
    else
        # Fallback: use the directory name or basename
...
# Function to check if folder should be processed
should_process_folder() {
    local folder_name="$1"
    
    # If --only is specified, only process folders in the list
    if [[ ${#ONLY_FOLDERS[@]} -gt 0 ]]; then
        for only_folder in "${ONLY_FOLDERS[@]}"; do
            if [[ "$folder_name" == "$only_folder" ]]; then
    ...
    # If --skip is specified, skip folders in the list
    if [[ ${#SKIP_FOLDERS[@]} -gt 0 ]]; then
...
# Find all .raw files (both in current directory and subdirectories)
declare -A processed_materials
raw_files_found=()

# Look for .raw files in current directory
for raw_file in *.raw; do
    if [[ -f "$raw_file" ]]; then
        raw_files_found+=("$raw_file")
    fi
done
...
echo "Found ${#raw_files_found[@]} .raw file(s)"

# Process each found .raw file
for raw_file in "${raw_files_found[@]}"; do
    if [[ ! -f "$raw_file" ]]; then
        continue
    fi
    
    # Extract material name and determine folder structure
    material_name=$(extract_material_name "$raw_file")
    raw_file_name=$(basename "$raw_file")
    base_name=$(basename "$raw_file" .raw)
    
    # Determine if this is in a subfolder or current directory
    raw_file_dir=$(dirname "$raw_file")
    if [[ "$raw_file_dir" == "." ]]; then
        # File is in current directory, use material name as folder
        folder="$material_name"
    else
        # File is in subdirectory, use that directory name
        folder="$(basename "$raw_file_dir")"
    fi
    
    echo "Found: $raw_file -> material: $material_name, folder: $folder"
    
    # Check if we should process this folder
    if ! should_process_folder "$material_name"; then
        echo "  Skipping $material_name (filtered by --skip or --only options)"
        continue
    fi
    
    # Avoid processing the same material multiple times
    if [[ -n "${processed_materials[$material_name]}" ]]; then
        echo "  Already processed $material_name, skipping duplicate"
        continue
    fi
    
    processed_materials[$material_name]=1
    ((total_folders++))
