...
def visualize_2d_compensation(comp, x_range, y_range, t_range, axes=['x', 'y']):
    """
    Visualize 2D compensation in X-T and Y-T spaces (learning from 2D code)
    ...
    Args:
        ...
        axes: list of axes to plot, can be ['x'], ['y'], or ['x', 'y'] (default both)
    """
    print(f"Visualizing 2D compensation in {'-'.join(axes).upper()}-T spaces...")
    
    # Compute compensation
    with torch.no_grad():
        compensation = comp(x_range, y_range, t_range)
        
        # Also get X and Y components separately
        t_shifted = t_range - t_range[0]
        memberships = comp.compute_window_memberships(x_range, y_range, t_shifted)
    ...
    for axis_idx, axis in enumerate(axes):
        base_idx = axis_idx * 4 if n_axes == 2 else 0
        
        if axis == 'x':
            # X-T view: average over Y dimension or use center Y
            y_center = len(y_range_np) // 2
            compensation_2d = compensation_np[:, y_center, :]  # [len_x, len_t]
            compensation_x_2d = compensation_x_np[:, y_center, :]
            compensation_y_2d = compensation_y_np[:, y_center, :]
            
            # Create X-T meshgrid for plotting
            X_2d, T_2d = np.meshgrid(x_range_np, t_range_np, indexing='ij')
            
            # Plot 1: Total compensation
            ax1 = axs[base_idx + 0]
            im1 = ax1.contourf(X_2d, T_2d/1000, compensation_2d, levels=30, cmap='coolwarm')
            plt.colorbar(im1, ax=ax1, label='Compensation (μs)')
            ...
            for b_idx in range(len(boundary_surfaces_np)):
                boundary_line = boundary_surfaces_np[b_idx][:, 0]
                color = colors[b_idx % len(colors)]
                ax1.plot(x_range_np, boundary_line/1000, '--', alpha=0.7, linewidth=1, color=color)
            
            ax1.set_xlabel('X (pixels)')
            ax1.set_ylabel('Time (ms)')
            ax1.set_title(f'Total Compensation X-T\n(Y={y_range_np[y_center]:.0f})')
            ax1.grid(True, alpha=0.3)
            
            # Plot 2: X-component only
            ax2 = axs[base_idx + 1]
            im2 = ax2.contourf(X_2d, T_2d/1000, compensation_x_2d, levels=20, cmap='Reds')
            plt.colorbar(im2, ax=ax2, label='X-Compensation (μs)')
            ax2.set_xlabel('X (pixels)')
            ax2.set_ylabel('Time (ms)')
            ax2.set_title(f'X-Component Only\n(small since ax≈0)')
            ax2.grid(True, alpha=0.3)
            
            # Plot 3: Slice at fixed X
            ax3 = axs[base_idx + 2]
            x_center = len(x_range_np) // 2
            ax3.plot(t_range_np/1000, compensation_2d[x_center, :], 'b-', linewidth=2, label='Total')
            ax3.plot(t_range_np/1000, compensation_x_2d[x_center, :], 'r--', linewidth=2, label='X-comp')
            ax3.plot(t_range_np/1000, compensation_y_2d[x_center, :], 'g--', linewidth=2, label='Y-comp')
            
            # Add boundary markers
            boundary_at_center = boundary_surfaces_np[:, x_center, 0]
            for b_idx, boundary_t in enumerate(boundary_at_center):
                if t_range_np.min() <= boundary_t <= t_range_np.max():
                    color = colors[b_idx % len(colors)]
                    ax3.axvline(boundary_t/1000, color=color, linestyle=':', alpha=0.7, label=f'B{b_idx}')
            
            ax3.set_xlabel('Time (ms)')
            ax3.set_ylabel('Compensation (μs)')
            ax3.set_title(f'Compensation vs Time\n(X={x_range_np[x_center]:.0f}, Y={y_range_np[y_center]:.0f})')
            ax3.grid(True, alpha=0.3)
            ax3.legend()
            
            # Plot 4: Slice at fixed T
            ax4 = axs[base_idx + 3]
            t_center = len(t_range_np) // 2
            ax4.plot(x_range_np, compensation_2d[:, t_center], 'b-', linewidth=2, label='Total')
            ax4.plot(x_range_np, compensation_x_2d[:, t_center], 'r--', linewidth=2, label='X-comp')
            ax4.plot(x_range_np, compensation_y_2d[:, t_center], 'g--', linewidth=2, label='Y-comp')
            ax4.set_xlabel('X (pixels)')
            ax4.set_ylabel('Compensation (μs)')
            ax4.set_title(f'Compensation vs X\n(T={t_range_np[t_center]/1000:.1f}ms, Y={y_range_np[y_center]:.0f})')
            ax4.grid(True, alpha=0.3)
            ax4.legend()
            
            print(f"  X-T view: Compensation range {compensation_2d.min():.1f} to {compensation_2d.max():.1f} μs")
            
        elif axis == 'y':
            # Y-T view: average over X dimension or use center X
            x_center = len(x_range_np) // 2
            compensation_2d = compensation_np[x_center, :, :]  # [len_y, len_t]
            compensation_x_2d = compensation_x_np[x_center, :, :]
            compensation_y_2d = compensation_y_np[x_center, :, :]
            
            # Create Y-T meshgrid for plotting
            Y_2d, T_2d = np.meshgrid(y_range_np, t_range_np, indexing='ij')
            
            # Plot 1: Total compensation
            ax1 = axs[base_idx + 0]
            im1 = ax1.contourf(Y_2d, T_2d/1000, compensation_2d, levels=30, cmap='coolwarm')
            plt.colorbar(im1, ax=ax1, label='Compensation (μs)')
            ...
            for b_idx in range(len(boundary_surfaces_np)):
                boundary_line = boundary_surfaces_np[b_idx][0, :]
                color = colors[b_idx % len(colors)]
                ax1.plot(y_range_np, boundary_line/1000, '--', alpha=0.7, linewidth=1, color=color)
            
            ax1.set_xlabel('Y (pixels)')
            ax1.set_ylabel('Time (ms)')
            ax1.set_title(f'Total Compensation Y-T\n(X={x_range_np[x_center]:.0f})')
            ax1.grid(True, alpha=0.3)
            
            # Plot 2: Y-component only
            ax2 = axs[base_idx + 1]
            im2 = ax2.contourf(Y_2d, T_2d/1000, compensation_y_2d, levels=20, cmap='Blues')
            plt.colorbar(im2, ax=ax2, label='Y-Compensation (μs)')
            ax2.set_xlabel('Y (pixels)')
            ax2.set_ylabel('Time (ms)')
            ax2.set_title(f'Y-Component Only\n(dominant since ay≈-70)')
            ax2.grid(True, alpha=0.3)
            
            # Plot 3: Slice at fixed Y
            ax3 = axs[base_idx + 2]
            y_center_idx = len(y_range_np) // 2
            ax3.plot(t_range_np/1000, compensation_2d[y_center_idx, :], 'b-', linewidth=2, label='Total')
            ax3.plot(t_range_np/1000, compensation_x_2d[y_center_idx, :], 'r--', linewidth=2, label='X-comp')
            ax3.plot(t_range_np/1000, compensation_y_2d[y_center_idx, :], 'g--', linewidth=2, label='Y-comp')
            
            # Add boundary markers
            boundary_at_center = boundary_surfaces_np[:, 0, y_center_idx]
            for b_idx, boundary_t in enumerate(boundary_at_center):
                if t_range_np.min() <= boundary_t <= t_range_np.max():
                    color = colors[b_idx % len(colors)]
                    ax3.axvline(boundary_t/1000, color=color, linestyle=':', alpha=0.7, label=f'B{b_idx}')
            ...
            ax3.set_xlabel('Time (ms)')
            ax3.set_ylabel('Compensation (μs)')
            ax3.set_title(f'Compensation vs Time\n(X={x_range_np[x_center]:.0f}, Y={y_range_np[y_center_idx]:.0f})')
            ax3.grid(True, alpha=0.3)
            ax3.legend()
            ...
            ax4 = axs[base_idx + 3]
            t_center = len(t_range_np) // 2
            ax4.plot(y_range_np, compensation_2d[:, t_center], 'b-', linewidth=2, label='Total')
            ax4.plot(y_range_np, compensation_x_2d[:, t_center], 'r--', linewidth=2, label='X-comp')
            ax4.plot(y_range_np, compensation_y_2d[:, t_center], 'g--', linewidth=2, label='Y-comp')
            ax4.set_xlabel('Y (pixels)')
            ax4.set_ylabel('Compensation (μs)')
            ax4.set_title(f'Compensation vs Y\n(T={t_range_np[t_center]/1000:.1f}ms, X={x_range_np[x_center]:.0f})')
            ax4.grid(True, alpha=0.3)
            ax4.legend()
            
            print(f"  Y-T view: Compensation range {compensation_2d.min():.1f} to {compensation_2d.max():.1f} μs")
    
    plt.tight_layout()
    plt.show()
    
    print(f"✓ 2D compensation visualization complete")
    print(f"✓ Dashed lines show window boundaries")
    print(f"✓ Contour plots show compensation strength")
    print(f"✓ Slice plots show compensation variation")
...
def test_realistic_scenario():
    ...
    print("\n" + "="*60)
    print("Step 2a: 2D Compensation Visualization (X-T and Y-T)")
    print("="*60)
    
    # Show 2D compensation plots
    visualize_2d_compensation(comp, x_range, y_range, t_range, axes=['x', 'y'])
    
    print("\n" + "="*60)
    print("Step 2b: Verify 3D Compensation")
    print("="*60)
    ...
    print(f"✓ 2D compensation visualization added (contours + slices)")
...
if __name__ == "__main__":
    ...
    print(f"   - Added 2D compensation visualization (contours + slices)")
    ...
    print(f"")
    print(f"Visualization Usage:")
    print(f"  # 2D membership for windows 1,5,9 in both X-T and Y-T")
    print(f"  visualize_2d_membership(comp, x, y, t, axes=['x','y'], windows=[1,5,9])")
    print(f"  # 2D compensation contours and slices")
    print(f"  visualize_2d_compensation(comp, x, y, t, axes=['x','y'])")
    print(f"")
    print(f"✓ OPTIMIZED: No for loops, vectorized operations")
    print(f"✓ CLEANED: No normalize option, minimal clamps")
    print(f"✓ ENHANCED: 2D membership + compensation visualization")
