...
\begin{figure*}[htbp]
  ...
  \begin{minipage}{0.24\linewidth}
    \phfig{(a) Optical path: LED $\rightarrow$ sample $\rightarrow$ grating scan $\rightarrow$ DVS/frame plane.}
  \end{minipage}\hfill
  \begin{minipage}{0.24\linewidth}
    \phfig{(b) Illustration of \eqref{eq:lambda_mapping}: wavelength fronts sweeping across $x$.}
  \end{minipage}\hfill
  \begin{minipage}{0.24\linewidth}
    \phfig{(c) Example $I_0(\lambda)$ with event polarities marking $\partial I_0/\partial \lambda$.}
  \end{minipage}\hfill
  \begin{minipage}{0.24\linewidth}
    \phfig{(d) Planned hardware photograph highlighting compact 4$f$ relay and sensors.}
  \end{minipage}
  \caption{Figure~1 placeholder: system overview and sensing concept. Panel (a) will depict the transmission optics, (b) the wavelength--time relationship, (c) the event-generation mechanism, and (d) the laboratory implementation.}
\label{fig:overview}
\end{figure*}
...
\begin{figure}[htbp]
  ...
  \begin{minipage}{0.48\linewidth}
    \phfig{(a) $x$--$t$ event projection before compensation, exhibiting temporal shear.}
  \end{minipage}\hfill
  \begin{minipage}{0.48\linewidth}
    \phfig{(b) $x$--$t$ projection after applying the multi-window warp.}
  \end{minipage}\\[6pt]
  \begin{minipage}{0.48\linewidth}
    \phfig{(c) Integrated scan images before/after warp (two panels in final figure).}
  \end{minipage}\hfill
  \begin{minipage}{0.48\linewidth}
    \phfig{(d) Estimated boundary surfaces $T_i(x)$ highlighting acceleration near turnarounds.}
  \end{minipage}
  \caption{Figure~3 placeholder: multi-window time-warp diagnostics and planned quantitative comparisons.}
  \label{fig:warp}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \begin{minipage}{0.48\linewidth}
    \phfig{(a) Monochrome reconstructions at 450, 550, and 650~nm.}
  \end{minipage}\hfill
  \begin{minipage}{0.48\linewidth}
    \phfig{(b) RGB composite derived from event data (with reference inset).}
  \end{minipage}\\[6pt]
  \begin{minipage}{0.48\linewidth}
    \phfig{(c) Spectral overlay: event reconstruction vs hyperspectral reference with residual band.}
  \end{minipage}\hfill
  \begin{minipage}{0.48\linewidth}
    \phfig{(d) Planned metrics: low-light RMS error and 1-pass vs 3-pass comparison.}
  \end{minipage}
  \caption{Figure~4 placeholder: spectral reconstruction, validation, and comparative analyses.}
  \label{fig:spectrum}
\end{figure}
