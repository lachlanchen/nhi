...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \end{scope}
  \end{tikzpicture}\\[-5pt]
  % Figure 3(b): 3D event cloud before compensation (tight-cropped)
  \begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (img3b) at (0,0)
      {\includegraphics[width=0.8\linewidth]{figures/event_cloud_before.pdf}};
    ...
    \end{scope}
  \end{tikzpicture}\\[-5pt]
  % Figure 3(c): 3D event cloud after compensation (tight-cropped)
  \begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (img3c) at (0,0)
      {\includegraphics[width=0.8\linewidth]{figures/event_cloud_after.pdf}};
    ...
    \end{scope}
  \end{tikzpicture}\\[-5pt]
  % Figure 3(d): Single 50 ms bin comparison (original vs compensated)
  \begin{tikzpicture}
    ...
    \begin{scope}[x={(img3d.south east)}, y={(img3d.north west)}]
      \node[font=\bfseries, anchor=north west, fill=none, text=black, inner sep=2pt] at (0.00, 0.900) {(d)};
    \end{scope}
  \end{tikzpicture}\\[-20pt]
  \caption{Multi-window compensation diagnostics. (a) Events (X--T and Y--T) with learned boundary surfaces overlaid. (b) 3D event cloud before compensation, tight-cropped to remove whitespace. (c) 3D event cloud after compensation showing the straightened motion plane. (d) Example 50~ms bin comparing original and compensated frames.}
  \label{fig:warp}
\end{figure}
