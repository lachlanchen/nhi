...
\begin{figure}[t]
  ...
  \begin{minipage}[t]{0.06\linewidth}
    \begin{tikzpicture}
      % Invisible reference to panel (d) height
      \node[anchor=south west, inner sep=0, opacity=0] (imgDref) at (0,0)
        {\includegraphics[width=\linewidth]{figures/overlay_image_after_plain.pdf}};
      \begin{scope}[x={(imgDref.south east)}, y={(imgDref.north west)}]
        % Normalize the local coordinate system to the full (d) panel
        \path[use as bounding box] (0,0) rectangle (1,1);
        \def\xb{0.15}\def\wb{0.25}
        \shade[bottom color=evtNeg, top color=white] (\xb,0.02) rectangle (\xb+\wb, 0.98/2);
        \shade[bottom color=white, top color=evtPos] (\xb,0.98/2) rectangle (\xb+\wb, 0.98);
        \node[anchor=west, inner sep=1pt, scale=0.7] at (\xb+\wb+0.03, 0.02) {$-1$};
        \node[anchor=west, inner sep=1pt, scale=0.7] at (\xb+\wb+0.03, 0.98/2) {$0$};
        \node[anchor=west, inner sep=1pt, scale=0.7] at (\xb+\wb+0.03, 0.98) {$1$};
      \end{scope}
    \end{tikzpicture}
  \end{minipage}\\[-2pt]
