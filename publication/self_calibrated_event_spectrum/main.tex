...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[x={(f2a.south east)}, y={(f2a.north west)}]
      % Panel label from LaTeX (no fill so background remains visible)
      \node[font=\bfseries, anchor=north west, inner sep=2pt] at (0.012, 0.988) {(a)};
    \end{scope}
  \end{tikzpicture}\\[-2pt]
  \begin{tikzpicture}
    ...
    \begin{scope}[x={(f2b.south east)}, y={(f2b.north west)}]
      % Panel label from LaTeX (no fill so background remains visible)
      \node[font=\bfseries, anchor=north west, inner sep=2pt] at (0.012, 0.988) {(b)};
    \end{scope}
  \end{tikzpicture}
  \vspace{-3pt}
