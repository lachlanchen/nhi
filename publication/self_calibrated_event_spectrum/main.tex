  ...
  \begin{minipage}[t]{0.45\linewidth}
    \begin{tikzpicture}
      \node[anchor=south west, inner sep=0] (img3d) at (0,0)
        {\includegraphics[width=0.94\linewidth]{figures/overlay_image_after_plain.pdf}};
      \begin{scope}[x={(img3d.south east)}, y={(img3d.north west)}]
        \node[font=\bfseries, anchor=north west, fill=none, text=black, inner sep=2pt]
          at (-0.05, 1.12) {(d)};
      \end{scope}
      % Color bar to the immediate right of (d), full height, with small gap
      \coordinate (barBL) at ($(img3d.south east)+(1.5mm,0mm)$);
      \coordinate (barTL) at ($(img3d.north east)+(1.5mm,0mm)$);
      \coordinate (barBR) at ($(barBL)+(3mm,0mm)$);
      \coordinate (barTR) at ($(barTL)+(3mm,0mm)$);
      \coordinate (barML) at ($(barBL)!0.5!(barTL)$);
      \coordinate (barMR) at ($(barBR)!0.5!(barTR)$);
      \shade[bottom color=evtNeg, top color=white] (barBL) rectangle (barMR);
      \shade[bottom color=white, top color=evtPos] (barML) rectangle (barTR);
      \node[anchor=west, inner sep=1pt, scale=0.7] at ($(barBR)+(0.6mm,0mm)$) {$-1$};
      \node[anchor=west, inner sep=1pt, scale=0.7] at ($(barMR)+(0.6mm,0mm)$) {$0$};
      \node[anchor=west, inner sep=1pt, scale=0.7] at ($(barTR)+(0.6mm,0mm)$) {$1$};
    \end{scope}
    \end{tikzpicture}
  \end{minipage}\\[-2pt]
