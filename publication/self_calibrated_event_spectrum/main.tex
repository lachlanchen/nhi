...
\author{Rongzhou Chen, Chutian Wang, Yuxing Li, Yuqing Cao, Shuo Zhu and Edmund Y. Lam{*}}
...
Our system consists of a dispersed illumination module and a detection module (Figure~\ref{fig:overview}). In the illumination path, a broadband LED with a slit and diffraction grating projects different wavelengths across the specimen. The transmitted light is collected and relayed (4$f$ system) to both an event camera and a frame sensor. This configuration preserves the original microscope imaging via a beam splitter and enables synchronous event and frame capture. The motor repeatedly scans forward and backward at an uncalibrated, non-uniform speed.
...
\end{equation}
which drives event emission when the logarithmic intensity crosses the sensor threshold~\cite{yu2025active}. Because $\partial \lambda/\partial t$ is governed by the motor trajectory, reconstructing $I_0^{(x,y)}(\lambda)$ reduces to integrating polarity-weighted events along a synchronized wavelength axis.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[x={(img3a.south east)}, y={(img3a.north west)}]
      ...
      \def\lx{0.90}\def\ly{0.97}\def\rr{0.014}
  ...
  % (c,d) 50 ms accumulations, side-by-side
  ...
  \begin{minipage}[t]{0.49\linewidth}
    \begin{tikzpicture}
      ...
      \begin{scope}[x={(img3d.south east)}, y={(img3d.north west)}]
        ...
        \def\xb{1.05}\def\yb{0.10}\def\wb{0.035}\def\hb{0.80}
