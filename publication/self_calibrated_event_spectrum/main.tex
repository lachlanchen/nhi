  ...
  \begin{minipage}[t]{0.06\linewidth}
    \begin{tikzpicture}
      ...
      \begin{scope}[x={(imgDref.south east)}, y={(imgDref.north west)}]
        \def\xb{0.10}\def\wb{0.30}
        \shade[bottom color=evtNeg, top color=white] (\xb,0.02) rectangle (\xb+\wb, 0.51);
        \shade[bottom color=white, top color=evtPos] (\xb,0.51) rectangle (\xb+\wb, 0.98);
        \node[anchor=west, inner sep=1pt, scale=0.7] at (\xb+\wb+0.03, 0.02) {$-1$};
        \node[anchor=west, inner sep=1pt, scale=0.7] at (\xb+\wb+0.03, 0.51) {$0$};
