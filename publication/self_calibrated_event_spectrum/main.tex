...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[x={(img3a.south east)}, y={(img3a.north west)}]
      ...
      \def\lx{0.90}\def\ly{0.95}\def\rr{0.014}
  ...
  % Measure panel (d) height to align the external color bar
  \newlength{\panelDheight}%
  \settoheight{\panelDheight}{\includegraphics[width=0.45\linewidth]{figures/overlay_image_after_plain.pdf}}%
  ...
  % Anchor bar height to panel (d) via an invisible reference image.
  \begin{minipage}[t]{0.06\linewidth}
    \begin{tikzpicture}
      % Reserve the bar area height equal to panel (d)
      \path[use as bounding box] (0,0) rectangle (0.06\linewidth, \panelDheight);
      % Draw full-height bar with small margins
      \def\xoff{0.012\linewidth}
      \def\barw{0.035\linewidth}
      \shade[bottom color=evtNeg, top color=white] (\xoff, 0.02\panelDheight)
        rectangle (\xoff+\barw, 0.51\panelDheight);
      \shade[bottom color=white, top color=evtPos] (\xoff, 0.51\panelDheight)
        rectangle (\xoff+\barw, 0.98\panelDheight);
      \node[anchor=west, inner sep=1pt, scale=0.7] at (\xoff+\barw+0.01, 0.02\panelDheight) {$-1$};
      \node[anchor=west, inner sep=1pt, scale=0.7] at (\xoff+\barw+0.01, 0.51\panelDheight) {$0$};
      \node[anchor=west, inner sep=1pt, scale=0.7] at (\xoff+\barw+0.01, 0.98\panelDheight) {$1$};
    \end{tikzpicture}
  \end{minipage}\\[-2pt]
