...
\begin{figure}[t]
  ...
  \end{tikzpicture}
  \vspace{-3pt}
  \caption{Self-synchronized scan segmentation. (a) Event counts over the full recording with pre- and post-scan shading and alternating forward and backward boundaries. (b) Auto-correlation and auto-convolution of the event activity signal; the red and blue marker indicates the peak used to determine the period, start and end of scanning.}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (img3a) at (0,0)
      ...
      {\includegraphics[width=0.8\linewidth]{figures/multiwindow_events_blank.pdf}};
    \begin{scope}[x={(img3a.south east)}, y={(img3a.north west)}]
      \node[font=\bfseries, anchor=north west, fill=none, text=black, inner sep=2pt] at (0.00, 1.050) {(a)};
    \end{scope}
  \end{tikzpicture}\\[1pt]
  ...
  \begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (img3b) at (0,0)
      ...
      {\includegraphics[width=0.8\linewidth]{figures/multiwindow_variance_blank.pdf}};
    \begin{scope}[x={(img3b.south east)}, y={(img3b.north west)}]
      \node[font=\bfseries, anchor=north west, fill=none, text=black, inner sep=2pt] at (0.00, 1.050) {(b)};
    \end{scope}
  \end{tikzpicture}\\[1pt]
  ...
  \begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (img3c) at (0,0)
      ...
      {\includegraphics[width=0.8\linewidth]{figures/multiwindow_bin50ms_sanqin.pdf}};
    \begin{scope}[x={(img3c.south east)}, y={(img3c.north west)}]
      \node[font=\bfseries, anchor=north west, fill=none, text=black, inner sep=2pt] at (0.00, 1.050) {(c)};
    \end{scope}
  \end{tikzpicture}
...
\begin{equation}
  ...
  \label{eq:rev_corr}
\end{equation}
exposes the pre- and post-scan regions through the mirror-symmetric peaks around the turnaround time. 

An iterative peak search refines the start and end indices of each half-scan, producing scanning segments without reference measurements. 
...
The fitted slopes span \(a \in\) \SIrange{-1.74}{5.50}{\micro\second\per\pixel} and \(b \in\) \SIrange{-79.82}{-75.67}{\micro\second\per\pixel}, representing the lateral and vertical time-per-pixel coefficients of the multi-window model.
...
\begin{figure}[t]
  ...
  \caption{Aligned background mapped to wavelength using the auto-alignment compared against a single spectrometer curve. }
  \label{fig:figure5}
\end{figure}
...
\end{equation}
the active scan interval $[t_0,t_1]$ is identified from the largest slope changes in the smoothed $\tilde b(t)$. For ground-truth spectra $\{g_i(\lambda)\}_{i=1}^M$ with active intervals $[\lambda_{0,i},\lambda_{1,i}]$, the time-to-wavelength mapping is given by the affine model
\begin{equation}
...
\hline
Acquisition time & \SIrange{150}{1100}{\milli\second} per scan & \SI{300}{\second} per scan \\
...
N.~Hagen and M.~W. Kudenov, Opt. Eng. \textbf{52}, 090901 (2013).

\bibitem{yeh2011selfcal}
C.-K. Yeh and V.~J. Tsai, ``Self-calibrated direct geo-referencing of airborne pushbroom hyperspectral images,'' in \emph{2011 IEEE International Geoscience and Remote Sensing Symposium} (IEEE, 2011), pp.~2881--2883.

\bibitem{alhourani2023linescan}
A.~Al-Hourani, S.~Balendhran, S.~Walia, and T.~Hourani, Remote Sens. \textbf{15}, 2787 (2023).

\bibitem{gallego2020event}
G.~Gallego, T.~Delbr\"uck, G.~Orchard, \emph{et al.}, IEEE Trans. Pattern Anal. Mach. Intell. \textbf{44}, 154 (2022).

\bibitem{ge2025event}
Z.~Ge, C.~Wang, J.~Huang, and E.~Y. Lam, Opt. Lett. \textbf{50}, 1496 (2025).

\bibitem{zhang2024neuromorphic}
P.~Zhang, S.~Zhu, C.~Wang, \emph{et al.}, IEEE Trans. Circuits Syst. Video Technol. (2024).

\bibitem{reinbacher2016manifold}
G.~Reinbacher, C.~Munda, and T.~Pock, IEEE Conf. Comput. Vis. Pattern Recognit. Workshops (2016).

\bibitem{bardow2016simultaneous}
P.~Bardow, A.~J. Davison, and S.~Leutenegger, ``Simultaneous optical flow and intensity estimation from an event camera,'' in \emph{Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR)} (IEEE, 2016), pp.~884--892.

\bibitem{mitrokhin2018mobility}
A.~Mitrokhin, C.~Ferm\"uller, C.~Parameshwara, and Y.~Aloimonos, ``Event-based moving object detection and tracking,'' in \emph{2018 IEEE/RSJ International Conference on Intelligent Robots and Systems (IROS)} (IEEE, 2018), pp.~1--9.

\bibitem{wang2025angle}
C.~Wang, S.~Zhu, P.~Zhang, \emph{et al.}, Laser \& Photonics Rev. \textbf{19}, 2400647 (2025).
