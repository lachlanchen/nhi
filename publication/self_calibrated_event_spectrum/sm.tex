...
\usepackage{placeins}
\usepackage{xcolor}
% Provide minimal citation macros so local builds never fail; natbib will override
...
\begin{figure}[t]
  \centering
  \includegraphicsSafe{figures_sm/device_setup}{\columnwidth}\\[-2pt]
  \caption{Device setup for the event-driven transmission hyperspectral microscope. The lower-left module is the scanning illumination arm, comprising a broadband 2835 LED, collimating optics, and a motorised grating--dichroic assembly driven by a NEMA-17 stepper motor. A dispersed illumination line is projected onto the specimen mounted at the centre of the breadboard. The vertical $4f$ detection column above the sample relays the transmitted light through a lateral beam splitter to an event camera and a frame-based sensor. Structural components in red/black are 3D-printed mounts, holders, and adapters, forming a compact, reconfigurable platform for self-synchronised spectral scanning.}
...
As a complementary, source-referenced validation, Fig.~\ref{fig:cumsum_vs_spd} compares calibrated spectral power distributions (SPDs) with background event statistics for two illumination sources. The top row shows the Nichia 2835 LED used for the microscope experiments, and the bottom row shows the Lumileds line source used for the self-calibration scan. For each source, the left panel overlays the normalized SPD (blue) with the event-based reconstruction obtained from background events (green), the middle panel plots their log-intensities, and the right panel compares the derivative of log-SPD with the dominant background event density measured from 5~ms bins. Across both sources, the background event curves closely follow the derivative of log intensity and, after integration and exponentiation, recover the SPD shape up to a global scale, consistent with the probabilistic trigger model.

\begin{figure}[t]
  \centering
  \includegraphicsSafe{figures_sm/three_panel_5ms_Scan_1_Forward_2835}{\columnwidth}\\[-2pt]
  \includegraphicsSafe{figures_sm/three_panel_5ms_Scan_1_Forward_lumileds}{\columnwidth}
  \caption{Comparison of calibrated spectral power distributions (SPDs) and background event statistics for two illumination sources. \textbf{Top:} Nichia 2835 LED (microscope experiments). \textbf{Bottom:} Lumileds line source (self-calibration scan). For each source, the left panel overlays the normalized SPD (blue) with the event-based reconstruction from background events (green), the middle panel plots their log-intensities, and the right panel compares the derivative of log-SPD with the dominant background event density measured in 5~ms bins. In both cases, the event curves are consistent with the probabilistic trigger model: the background density tracks the derivative of log intensity, and its integrated-and-exponentiated form recovers the SPD shape up to a global scale.}
...
\olsection{Segmentation and compensation parameters}
For reproducibility, we summarize the operating points used in the segmentation and multi-window compensation stages. All commands were executed in the \texttt{nhi\_test} conda environment and use the forward scan \texttt{Scan\_1\_Forward\_events.npz} from the Sanqin 2835 dataset.
\begin{itemize}
  \item \textbf{Segmentation.} We ran \texttt{segment\_robust\_fixed.py} with automatic period estimation (\texttt{--auto\_calculate\_period}) and event segmentation enabled (\texttt{--segment\_events}). The bin width for edge finding was \SI{50}{\milli\second} (\texttt{--bin\_width\_us 50000}), and the sensor geometry was fixed to \SI{1280}{\pixel} $\times$ \SI{720}{\pixel} to match the Prophesee sensor. Outputs were written to the per-scan segment folder specified via \texttt{--output\_dir}.
  \item \textbf{Compensation.} The compensated time stamps were learned with \texttt{compensate\_multiwindow\_train\_saved\_params.py} using the same \SI{50}{\milli\second} bin width (\texttt{--bin\_width 50000}), trainable $a$-slopes (\texttt{--a\_trainable}), and \texttt{--iterations 1000}. We kept the polarity weights at their defaults (\texttt{pos\_scale = 1.0}, negative weight auto-selected) and used a fine rescaling step of \SI{5}{\milli\second} in downstream visualization. The learned parameters were saved next to the segment NPZ and reused for all figures in this supplement.
\end{itemize}
These settings ensure the segmentation boundaries, compensation surfaces, and subsequent wavelength alignment are fully determined by the recorded data and the published scripts, with no manual tuning between figures.
