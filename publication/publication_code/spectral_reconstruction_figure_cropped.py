...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        choices=("lower", "upper"),
        default="lower",
        help="Origin used when plotting externally provided PNGs (diff/ref rows). Default matches legacy behavior ('lower').",
    )
...
def _parse_start_nm(name: str) -> float | None:
    m = re.match(r".*_(\d+(?:\.\d+)?)nm(?:_[^.]*)?\.(?:png|jpg|jpeg)$", name, flags=re.IGNORECASE)
    if m:
        try:
...
def apply_crop_and_flip(img: np.ndarray, bbox: Tuple[int, int, int, int] | None, flip: bool) -> np.ndarray:
    processed = img
    if flip:
...
def render_grid_downsampled(
    ...
    row_config: Dict[str, Dict[str, object]] | None = None,
) -> None:
    setup_style()
    ...
    def save_frame(path: Path, data: np.ndarray, cmap: Colormap, vmin: float | None = None, vmax: float | None = None, norm: TwoSlopeNorm | None = None) -> None:
        if norm is not None:
            normed = norm(data)
        else:
    ...
    orig_used_dir = output_dir / "orig_used_frames"
    if orig_used_dir.exists():
        shutil.rmtree(orig_used_dir)
    orig_used_dir.mkdir(parents=True, exist_ok=True)

    comp_used_dir = output_dir / "comp_used_frames"
    if comp_used_dir.exists():
        shutil.rmtree(comp_used_dir)
    comp_used_dir.mkdir(parents=True, exist_ok=True)

    diff_used_dir = None
    if diff_paths_sel:
        diff_used_dir = output_dir / "diff_used_frames"
        if diff_used_dir.exists():
            shutil.rmtree(diff_used_dir)
        diff_used_dir.mkdir(parents=True, exist_ok=True)

    ref_used_dir = None
    if has_gt_images:
        ref_used_dir = output_dir / "ref_used_frames"
        if ref_used_dir.exists():
            shutil.rmtree(ref_used_dir)
        ref_used_dir.mkdir(parents=True, exist_ok=True)

    # Populate image data
    for i, (orig_frame, comp_frame, meta) in enumerate(kept):
        nm_label = None
        if gt_wavelength_lookup and meta["index"] in gt_wavelength_lookup:
            nm_label = float(gt_wavelength_lookup[meta["index"]])
        elif wavelength_lookup and meta["index"] in wavelength_lookup:
            nm_label = float(wavelength_lookup[meta["index"]])
        fname_stub = f"bin_{meta['index']:02d}"
        if nm_label is not None:
            fname_stub += f"_{nm_label:.0f}nm"
        cfg_orig = row_config.get("orig", {})
        cfg_comp = row_config.get("comp", {})
        orig_view = apply_crop_and_flip(orig_frame, cfg_orig.get("crop"), cfg_orig.get("flip", False))
        comp_view = apply_crop_and_flip(comp_frame, cfg_comp.get("crop"), cfg_comp.get("flip", False))
        raw_vmin = min(0.0, float(orig_view.min()))
        raw_vmax = max(0.0, float(orig_view.max()))
        comp_vmin = min(float(comp_view.min()), 0.0)
        comp_vmax = max(float(comp_view.max()), 0.0)
        ...
        ax0 = axes[0, i]
        ax0.imshow(orig_view, cmap=raw_cmap, vmin=raw_vmin, vmax=raw_vmax, origin="lower")
        ax0.axis("off")
        ax0.set_xticks([])
        ax0.set_yticks([])
        save_frame(orig_used_dir / f"{fname_stub}.png", orig_view, raw_cmap, vmin=raw_vmin, vmax=raw_vmax)
        ax1 = axes[1, i]
        ax1.imshow(comp_view, cmap=comp_cmap, vmin=None if comp_norm else comp_vmin, vmax=None if comp_norm else comp_vmax, norm=comp_norm, origin="lower")
        ax1.axis("off")
        ax1.set_xticks([])
        ax1.set_yticks([])
        save_frame(comp_used_dir / f"{fname_stub}.png", comp_view, comp_cmap, vmin=comp_vmin, vmax=comp_vmax, norm=comp_norm)
        # Column wavelength labels are now handled exclusively by the bottom spectrum bar

        cfg_grad = row_config.get("grad", {})
        cfg_ref = row_config.get("ref", {})
        ...
        if diff_paths_sel and i < len(diff_paths_sel) and diff_paths_sel[i] and diff_paths_sel[i].exists():
            ...
            img_diff_view = apply_crop_and_flip(img_diff, cfg_grad.get("crop"), cfg_grad.get("flip", False))
            ...
            diff_frame = apply_crop_and_flip(
                comp_frame.astype(np.float32) - orig_frame.astype(np.float32),
                cfg_grad.get("crop"),
                cfg_grad.get("flip", False),
            )
        ...
        if has_gt_images:
            ...
            if ref_paths_sel and i < len(ref_paths_sel):
                ...
                img_ref_view = apply_crop_and_flip(img_ref, cfg_ref.get("crop"), cfg_ref.get("flip", False))
                origin_mode = external_origin
                ...
                blank_ref = apply_crop_and_flip(np.zeros_like(comp_frame), cfg_ref.get("crop"), cfg_ref.get("flip", False))
    ...
    for _, _, meta in kept:
        ...
        bar_colors.append(np.tile(rgb[None, None, :], (int(bar_px), block_width, 1)))
        bar_labels.append(nm_val)
    if bar_colors:
        valid_labels = [lbl for lbl in bar_labels if lbl is not None]
        use_gradient = len(valid_labels) >= 2 and not np.isclose(max(valid_labels), min(valid_labels))
        if use_gradient:
            nm_min = min(valid_labels)
            nm_max = max(valid_labels)
            total_width = block_width * len(bar_labels)
            nm_samples = np.linspace(nm_min, nm_max, total_width)
            gradient_colors = np.stack([nm_to_rgb_color(float(nm), REPO_ROOT) for nm in nm_samples], axis=0)
            bar_img = np.tile(gradient_colors[None, :, :], (int(bar_px), 1, 1))
        else:
        ...
        ax_bar.set_yticks([])
        ax_bar.set_ylim(0, bar_px)

        if any(lbl is not None for lbl in bar_labels):
            if use_gradient:
                total_width = bar_img.shape[1]
                nm_min = min(valid_labels)
                nm_max = max(valid_labels)
                denom = nm_max - nm_min if not np.isclose(nm_max, nm_min) else 1.0
                centres = [((lbl - nm_min) / denom) * (total_width - 1) if lbl is not None else np.nan for lbl in bar_labels]
                xticks = [c for c in centres if not np.isnan(c)]
                xticklabels = [f"{lbl:.0f}" for lbl in bar_labels if lbl is not None]
                ax_bar.set_xlim(0, total_width)
            else:
                xticks = list((np.arange(len(bar_labels)) + 0.5) * block_width)
                xticklabels = [f"{lbl:.0f}" if lbl is not None else "" for lbl in bar_labels]
                ax_bar.set_xlim(0, block_width * max(len(bar_labels), 1))
            ax_bar.set_xticks(xticks)
            ax_bar.set_xticklabels(xticklabels, fontsize=11, fontweight="bold")
            ax_bar.tick_params(axis="x", which="both", bottom=False, top=False, labelbottom=True, pad=6)
            ax_bar.set_xlabel("Wavelength (nm)", fontsize=11, labelpad=12)
    ...
    if figure_name:
        ...
        stem = "spectral_reconstruction_scan"
...
def main() -> None:
    ...
    tpl_bbox: Tuple[int, int, int, int] | None = None
    if args.crop_json and args.crop_json.exists():
        crop_meta = json.loads(args.crop_json.read_text())
        ref_meta = crop_meta.get("ref_crop")
        tpl_meta = crop_meta.get("template_crop")
        if ref_meta and all(k in ref_meta for k in ("x0", "y0", "x1", "y1")):
            ref_bbox = (int(ref_meta["x0"]), int(ref_meta["y0"]), int(ref_meta["x1"]), int(ref_meta["y1"]))
        if tpl_meta and all(k in tpl_meta for k in ("x0", "y0", "x1", "y1")):
            tpl_bbox = (int(tpl_meta["x0"]), int(tpl_meta["y0"]), int(tpl_meta["x1"]), int(tpl_meta["y1"]))

    row_config = {
        "orig": {"crop": ref_bbox, "flip": args.flip_row12},
        "comp": {"crop": ref_bbox, "flip": args.flip_row12},
        "grad": {"crop": tpl_bbox, "flip": args.flip_row34},
        "ref": {"crop": tpl_bbox, "flip": args.flip_row34},
    }
    ...
    bar_ratio = float(args.bar_height_ratio)
    if args.show_wavelength:
        bar_ratio = max(bar_ratio, 0.18)
